...
Команда `GET_LOG` используется для удаленного скачивания файлов логов процессов, сохраненных на SD-карте контроллера под именами, соответствующими номеру процесса (например, `15.CSV`).

Максимальная длина строки запроса — 30 символов, включая завершающие строку `\n` и нулевой символ.

#### Особенности выполнения команды

* **Блокировка телеметрии**: На время выполнения запроса регулярная отправка фоновой телеметрии приостанавливается. Это исключает коллизии в общем буфере UART. Отправка телеметрии автоматически и бесшовно возобновляется сразу после отправки маркера завершения или ошибки.
* **Лимиты буфера передачи 900 байт**:
* В **режиме вывода файла** данные передаются порциями (чанками) размером не более **898 байт** за один раз. Принимающая сторона должна быть готова обрабатывать строки такой длины.
* В **режиме списка файлов** размер итогового JSON-пакета гарантированно **не превышает 900 байт** (строго контролируется прошивкой). Если файлов на карте памяти больше, чем может вместить буфер, список будет безопасно обрезан на последнем поместившемся файле без нарушения валидности JSON.
...
Если номер процесса указан: запускается потоковая передача содержимого выбранного файла.
...
#### Формат ответа списка файлов
Ответ (один JSON-пакет, содержащий массив имен файлов):
...
#### Формат ответа при успешной передаче файла
Ответ представляет собой непрерывный текстовый поток, разделенный на три фазы:

1. **Маркер начала передачи (`START`)**:
   ```json
   {"type":"response","request":"GET_LOG","result":"START","file":"15.CSV"}
   ```

2. **Тело файла**:
   Сразу после маркера `START` контроллер транслирует сырое содержимое CSV-файла «как есть» построчно:
   ```csv
V;2.3.28
H;0,25
PRE;0,13
...
V2;12000
V3;13500
Этап;Время;Т.отбора;Клапан открыт;Период;Скорость отбора;Давление;Давление в кубе;ТД1;ТД2;ТД1 коррекция;ТД2 коррекция;ГУО, с;УО, с;СУО, с;Спиртуозность;Остановка;Всего остановок;Упреждений
1;0:00:00;0,00;9,40;100;9,40%;741,3;751,3;77,50;83,19;78,14;83,49;0000;0000;0000;44,0;0;0;0
1;0:01:00;0,00;9,40;100;9,40%;741,3;751,3;77,50;83,19;78,14;83,49;0009;0000;0000;44,0;0;0;0
   ```

3. **Маркер завершения передачи (`EOF`)**:
   ```json
   {"type":"response","request":"GET_LOG","result":"EOF"}
   ```
...
Если передача не может быть начата или прерывается в процессе, возвращается JSON-строка с кодом ошибки. Коды ошибок файловой системы FatFS транслируются напрямую в поле `code`.

| Ошибка (`result`) | Описание | Возможная причина |
| :--- | :--- | :--- |
| `err_num` | Неверный формат | Не указан или некорректно задан номер лога в запросе. |
| `err_mnt` | Ошибка монтирования | Не удалось смонтировать SD-карту (код ошибки в `code`, например `3` — карта отсутствует). |
| `err_nf` | Файл не найден | Лога с таким номером на SD-карте нет (код ошибки в `code`, обычно `8`). |
| `err_int` | Передача прервана | Ошибка чтения данных с карты или аппаратный таймаут UART во время чтения файла. |

*Пример ответа при отсутствии файла (ошибка 8 — `FR_NO_FILE`):*
```json
{"type":"response","request":"GET_LOG","result":"err_nf","code":8}
```
